/* =========================================================
   ✅ CODE 1: FRONT-END Projects component (Tabs + Dialog + Carousel)
   - Safe if technologies/images/category/live_url are null
   - Shows ALL projects (no show more)
   - “Shopify” tab (instead of Freelance)
   - Accepts both old category values: "Freelance"/"freelance" ✅ AND new "shopify"
   ========================================================= */
...
type Category = "wordpress" | "coding" | "design" | "shopify";
...
interface ProjectRow {
  ...
  category: string | null; // can be "Freelance"/"freelance"/"shopify"/etc
...
const normalizeCategory = (v: unknown): Category => {
  ...
  if (c === "freelance") return "shopify"; // ✅ old freelance becomes Shopify
  return "coding"; // fallback so it still appears
};
...
const Projects = () => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      ...
      // 2) fallback if created_at missing / blocked
      if (error) {
      ...
      // if we fetched without DB ordering, sort here
      normalized.sort((a, b) => {
  ...
  const shopifyProjects = useMemo(
    () => projects.filter((p) => p.category === "shopify"),
    [projects]
  );
      ...
      <div className="container mx-auto px-4">
        ...
        <Tabs defaultValue="coding">
          <TabsList className="mb-6 justify-center">
            ...
            {/* ✅ Shopify tab appears only if there is data */}
          ...
          {shopifyProjects.length > 0 && (
            <TabsContent value="shopify">
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {shopifyProjects.map(renderProjectCard)}
              </div>
            </TabsContent>
          )}
        </Tabs>
